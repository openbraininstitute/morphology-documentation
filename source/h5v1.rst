...
The datasets in the root `/` are:
  ...
  - `structure`
     ...
     - `type`: The type of process for this section. This integer is
       ...
       For multiple spines the values are: 1: `spine` (generic), 2: `neck`, 3: `head`.
...
morphology, following the same specification. The key difference with the
single morphology is that only the spines are represented. In this case, there
is no soma nor neurites. The spines' coordinates (`points` structure) can be
local (centered at (0,0,0)) or global (with respect to its corresponding
morphology points). Therefore, each spine is an independent tree structure.
...
For the example, we will be using a set of two spines that illustrate the
different use cases supported by the format:

- **Spine A** (simple): A spine with a neck section (Green) followed by a head
  section (Red). This represents the most common case of a non-branching spine.

- **Spine B** (bifurcated): A spine whose neck (Blue) bifurcates into two head
  sections (Orange and Purple). This represents a more complex spine where the
  head region branches.

Note that the points belonging to different spines do not need to be connected
(their coordinates are independent). Shared points at bifurcation boundaries
are duplicated, as in the neuron morphology structure.
...
diagram):
   ...
   |   0  |   1.  |   1.  |   0.  |  0.1  |                           |
   +------+-------+-------+-------+-------+                           |
   |   1  |   2.  |   2.  |   0.  |  0.15 |  Green (Spine A neck)     |
   +------+-------+-------+-------+-------+                           |
   |   2  |   3.  |   3.  |   0.  |  0.2  |                           |
   +------+-------+-------+-------+-------+---------------------------+
   |   3  |   3.  |   3.  |   0.  |  0.3  |                           |
   ...
   |   5  |   5.  |   5.  |   0.  |  0.1  |                           |
   +------+-------+-------+-------+-------+                           |
   |   6  |   5.  |   6.  |   0.  |  0.15 |  Blue (Spine B neck)      |
   +------+-------+-------+-------+-------+                           |
   |   7  |   5.  |   7.  |   0.  |  0.2  |                           |
   +------+-------+-------+-------+-------+---------------------------+
   |   8  |   5.  |   7.  |   0.  |  0.3  |                           |
   +------+-------+-------+-------+-------+  Orange (Spine B head 1)  |
   |   9  |   4.  |   8.  |   0.  |  0.7  |                           |
   +------+-------+-------+-------+-------+---------------------------+
   |  10  |   5.  |   7.  |   0.  |  0.3  |                           |
   +------+-------+-------+-------+-------+  Purple (Spine B head 2)  |
   |  11  |   6.  |   8.  |   0.  |  0.7  |                           |
   +------+-------+-------+-------+-------+---------------------------+
...
- Each section with `PID = -1` marks the root of a new spine. This is how
  individual spines are distinguished within the file: every root section
  starts a separate spine tree.

- Sections 0 and 1 belong to **Spine A**. Section 0 (green) is the neck (type
  2) and section 1 (red) is the head (type 3). The shared point at the
  neck/head boundary (3., 3.) is duplicated as points `i=2` and `i=3`.

- Sections 2, 3, and 4 belong to **Spine B**. Section 2 (blue) is the neck
  (type 2). Sections 3 (orange) and 4 (purple) are both heads (type 3) that
  branch from section 2 (`PID = 2`), forming a bifurcation. The shared point at
  the bifurcation (5., 7.) is duplicated as points `i=7`, `i=8`, and `i=10`
  (the last point of the neck and the first point of each head).
