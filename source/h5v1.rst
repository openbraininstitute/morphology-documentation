...
Multiple spines: Here multiple spines are represented within one logical
morphology for efficiency reasons: only a single `points` and `structure`
dataset is required. Since it is a spine, no soma or neurites are included.
The spine starts are denoted by their parent ID being -1. The spines'
coordinates (`points` structure) can be local (centered at (0,0,0)) or global
(with respect to its corresponding morphology).
